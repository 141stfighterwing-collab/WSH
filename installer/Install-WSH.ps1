...
function Start-ApplicationDeployment {
    ...
    try {
        # Try to pull pre-built image from GitHub Container Registry first (much faster)
        Write-ProgressDetail -Activity "Deploying Application" -Status "Pulling WSH image from GitHub Container Registry..." -PercentComplete 10
        Write-SubStep "Pulling pre-built WSH image (this is much faster than building locally)..." -Status running
        
        $imageName = "ghcr.io/141stfighterwing-collab/wsh:latest"
        ...
        if ($pullSuccess) {
            Write-SubStep "WSH image pulled successfully from GitHub Container Registry!" -Status success
            Write-Log "WSH image pulled from ghcr.io" -Level SUCCESS
            
            # Image is already set in docker-compose.yml, just start it
            ...
            Write-SubStep "Pull failed, falling back to local build..." -Status warning
            Write-Log "Image pull failed, building locally instead" -Level WARNING
            
            # Clone repository and build locally
            Write-ProgressDetail -Activity "Deploying Application" -Status "Cloning WSH repository..." -PercentComplete 20
            ...
            Write-ProgressDetail -Activity "Deploying Application" -Status "Building WSH Docker image..." -PercentComplete 40
            Write-SubStep "Building WSH Docker image locally (this may take a few minutes)..." -Status running
            
            Push-Location $repoPath
            
            $buildResult = docker build -t wsh-app:latest . 2>&1
            ...
            Write-ProgressDetail -Activity "Deploying Application" -Status "Starting WSH application..." -PercentComplete 60
            Write-SubStep "Starting WSH application container..." -Status running
        ...
        while ($retries -lt $maxRetries) {
            ...
            Write-ProgressDetail -Activity "Deploying Application" -Status "Waiting for application... ($retries/$maxRetries)" -PercentComplete $progressPercent
            
            if ($retries % 10 -eq 0) {
                Write-Host "  Container Status: $containerStatus" -ForegroundColor DarkGray
                docker logs $script:Config.AppContainer --tail 3 2>&1 | ForEach-Object { Write-Host "  $_" -ForegroundColor DarkGray }
            }
